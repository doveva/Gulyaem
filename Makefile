...
ASKCEL_RUNTIME_CONTRACT_VERSION ?= 1
ASKCEL_PRODUCT ?= walking
ASKCEL_WORKLOAD ?= walking-api
ASKCEL_ENVIRONMENT ?= $(ENVIRONMENT)
ASKCEL_RELEASE ?= local
ASKCEL_VERSION ?= dev
ASKCEL_INSTANCE ?=
ASKCEL_BUILD_COMMIT ?=
...
export ASKCEL_RUNTIME_CONTRACT_VERSION ASKCEL_PRODUCT ASKCEL_WORKLOAD ASKCEL_ENVIRONMENT ASKCEL_RELEASE ASKCEL_VERSION
export ASKCEL_INSTANCE ASKCEL_BUILD_COMMIT OTEL_EXPORTER_OTLP_ENDPOINT OTEL_EXPORTER_OTLP_PROTOCOL
...
bootstrap:
	cd backend && GOPRIVATE="github.com/Asksel-Ecosystem/*" go mod download
	cd frontend && npm ci
